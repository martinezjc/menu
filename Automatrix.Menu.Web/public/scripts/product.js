...
/*
* Change event handler for Company select when add a new product
*/
...
/*
* Change event handler for Company select when edit a product
*/
...
/*
* Change event handler for Product Name select when add a new product
*/
...
/*
* Change event handler for Product Name select when edit a product
*/
...
function loadCompanyProducts(companyId, typeSelected)
{
    ...
    ajaxBuilder( currentUrl + '/loadCompanyProducts', parameters, function(companyProducts) 
    {
        ...
        var data = JSON.parse(companyProducts);
        var i = 0;
        $.each(data, function () {
...
/*
* Loads dynamic options select when a product is selected
*/
...
/*
* Enable / disable fields to use in the menu options when the product uses Web Service option
* Type, Term, Deductible, Vehicle Plan, Mileage, Tire Rotation
*/
...
/*
* Show and hide options: width and height for the brochure field 
*/
...
/*
* Event click handler for Checkbox when the product uses webservice 
*/
...
// Range pricing functions

/* Checkbox event handler when the product have manual pricing */
...
$('#saveRangePricing').click( function() 
{
  ...
  if ( empty == true)
  {
    ...
    ajaxBuilder( currentUrl + '/saveRangePricing', parameters, function() {
...
$('#updateInfo').click(function () {
    ...
    selectedTab = $('ul#optionsCostPrice li.active').text();
    SellingPrice = $('#sellingPriceModified').val().replace('$', '');
    Cost  = $('#costModified').val().replace('$', '');
    emptyBullets = true;
    ...
    ajaxBuilder(currentUrl + '/dealers/' + $('#DealerId').val() + '/products/' + $('#idModified').val() +'/update', parameters, function(updateProductData){
        uploadModifiedData(function (updateProductData) {
            if (updateProductData == true) {
                if ( $('#PDFSelected').val() != 'yes') {
                    $.unblockUI(); 
                    toastr.success("The product has been updated.", "Success");
                    setTimeout(function(){
                        window.location.href = currentUrl + '/dealers/' + $('#DealerId').val() + '/products';
                    },3000);
                }
            }
        });
...
$('#saveProduct').click(function () {
    ...
    switch (selectedTab) 
    {
        case 'Manual': SellingPrice = $('#sellingPrice').val().replace('$', '');
            ...
            break;
        case 'Web Service': SellingPrice = 0;
            Cost = 0;
            SellingPrice = GetFloat(SellingPrice);
            Cost = GetFloat(Cost);
            UsingWebServiceInfo = 1;
    ...
    ajaxBuilder(currentUrl + '/dealers/' + $('#DealerId').val() + '/products/create', parameters, function(insertedId) {
        resetInputs();
        saveDocument(insertedId, function (callback) {
            if (callback == true) {
                if ( $('#PDFSelected').val() != 'yes') {
                    $.unblockUI();
