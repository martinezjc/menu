...
class ProductsController extends BaseController
{
	...
	public function get_ShowProductsViews()
	{
		...
		$rangePricing = DB::table('ProductPrice')
					 ->get ();
						...
						if($product->UsingWebService)
						{
							...
							if ($product->CompanyId == 1) {
									...
									foreach ( $rates ['Rate'] as $key => $rate )
									{
										...
										else if(array_key_exists('EndMonthTerm', $rate)){
		...
		return View::make ( 'financemenu' )->with ( 'Products', $products )->with ( 'Products2', $products2 )->with ( 'Products3', $products3 )->with ( 'Products4', $products4 )->with ( 'Datas', $productDetail )->with ( 'Settings', $settings )->with ( 'PlansProducts', $plansProducts )->with ('ShowPrintButton',false)->with ('FailWebservice',$FailWebservice)->with ('ShowMenuPrintButton',true)->with ('deal',$deal);
		} else {
	...
	private function getMatchingRate($product, $rates)
	{
		...
		if ($product->CompanyId == 1) { // US Warranty
        ...
        if ($product->CompanyId == 2) // Protective
        {
            ...
            // By Default returns the first rate
            return array($rates['AutomobileRateQuote'][0], 0);
		...
		if ($product->CompanyId == 3) // Road Vantage
		{
            ...
            return array($rates, $rates->TermMile->TermId);
		}
		return null;
	}
	...
	{	
		/*
		* var type of request
		* get price of product = 0
		* get contract = 1
		*/
		$company = DB::table ( 'Company' )->select('Username', 'Password')->where('id', '=', $product->CompanyId)
		->first();

		$deal->Username = $company->Username;
		$deal->Password = $company->Password;

		$response = $proxy->execute(new Api\Services\Request(array("product" => $product, "deal" => $deal, "dealercode" => $dealercode, "productOptions" => $productOptions,"productRates" => $productRates,"type" => $type)));
	...
	private function getProxy($dealer, $product)
	{
		...
		if ($product->CompanyId == 3 && ($dealer->IsPDF == 1)) {
			$company = DB::table ( 'Company' )->select('id', 'CompanyName','Username', 'Password','URL2 as URL')->where('id', '=', $product->CompanyId)
			->first();
		}else{
			$company = DB::table ( 'Company' )->select('id', 'CompanyName','Username', 'Password','URL')->where('id', '=', $product->CompanyId)
			->first();
		}
	...
	public function update($id)
	{
		...
		return Redirect::route ( 'Products.edit', $id )->withInput ()->withErrors ( $validation )->with ( 'message', 'There were validation errors.' );
	}
	...
	public function get_infoProduct()
	{
		...
		$Product = DB::table('Products')
         ...
         $data[] = array('idModified' => $Product->id,
            ...
            //if ( $Product->UseRangePricing == 1) {
            	$Ranges = DB::table('ProductPrice')
            	->where('ProductId', '=', $Product->id)
            	->get();
            
                if ( $Ranges ) {
	            	foreach ($Ranges as $Range => $RangeInfo) {
	            		$data[]['PricingInfo'] = array( 'idPricing' => $RangeInfo->ProductPriceId, 
    ...
    public function insertRangePricing()
    {
        ...
        if ( $ProductBaseId == 11 ) { 
			...
			if ($rangePricing == 1)
			{
				$ProductUpdated = DB::table ( 'Products' )->where ( 'id', $ProductId )
				->update ( array ('UseRangePricing' => $rangePricing,
					               'UsingWebService' => 0,
					               'Type' => $Type) );
				if ( $empty == false ) 
				{
	...
	{
		$UserSessionInfo = Session::get('UserSessionInfo');
		$Products = DB::table ( 'Products' )->join('ProductBase', 'Products.ProductBaseId', '=', 'ProductBase.ProductBaseId')->join ( 'PlansProducts', 'Products.id', '=', 'PlansProducts.ProductId' )->where('PlansProducts.DealerId', '=', $UserSessionInfo->DealerId)->where('Products.DealerId', '=', $UserSessionInfo->DealerId)->orderBy ( 'PlansProducts.Order', 'asc' )->get ();
		...
		$Products2 = DB::select ( DB::raw ( "select *
					...
					" ), array('value' => $UserSessionInfo->DealerId)  );
		...
		foreach ( $Products as $PlansProduct => $Plans )
		{
				...
				foreach ( $Products as $Product => $ProductInfo )
				{
					if ($ProductInfo->id == $ProductContainerPlan)
					{
						...
						// Start Print Table
						$table .= '<tr>
						          <td ><input type="checkbox" checked  name="productRef' 
						          . $ProductInfo->id . '" id="' . $ProductInfo->id 
						          . '" value="' . $ProductInfo->id 
						          . '  class="m" ">  '. ++ $PlansProduct .'</td>
						          <td>' . $productCompanyName . '</td>';
						          $table .= '<td><a href="edit-product?ProductId=' . $ProductInfo->id . '" name="' 
						          . $ProductInfo->id . '" class="modify"><span title="Edit Product">' 
						          . $ProductName . '  <i class="fa fa-pencil-square-o"></i></span></a><br/>';
		...
		foreach ( $Products2 as $Products2key => $Products2value )
		{
			...
			if ($flag == 0)
			{
				...
				// Start Print Table
				$table .= '<tr>
						          <td><input type="checkbox" name="productRef' . $Products2value->id 
						          . '" id="' . $Products2value->id . '" value="' 
						          . $Products2value->id . '"></td>
						          <td>' . $productCompanyName . '</td>';
						          $table .= '<td><a href="edit-product?ProductId=' . $Products2value->id 
						          . '" name="' . $Products2value->id . '" class="modify"><span title="Edit Product">' 
						          . $ProductName . '  <i class="fa fa-pencil-square-o"></i></span></a><br/>';
	...
	{
		$UserSessionInfo = Session::get('UserSessionInfo');
		$Products = DB::table ( 'Products' )->join ( 'PlansProducts', 'Products.id', '=', 'PlansProducts.ProductId' )->where('PlansProducts.DealerId', '=', $UserSessionInfo->DealerId)->where('Products.DealerId', '=', $UserSessionInfo->DealerId)->orderBy ( 'PlansProducts.Order', 'asc' )->get ();
		$Company = DB::table ( 'Company' )->get ();
...
//		$ProductsInverted = DB::table ( 'Products' )->join ( 'PlansProducts', 'Products.id', '=', 'PlansProducts.ProductId' )->orderBy ( 'PlansProducts.Order', 'desc' )->get ();
		...
		return View::make ( 'disclosureMenu' )->with ( 'Products', $Products )->with ( 'Details', $ProductDetail )
	...
	public function loadCompanyProducts()
	{
		...
		{
			foreach ($Products as $Product => $ProductInfo) {
				$data[] = array('ProductBaseId' => $ProductInfo->ProductBaseId,
					            'ProductName' => $ProductInfo->ProductName);
			}
		}
		else
		{
			$data[] = array('ProductBaseId' => '0',
				            'ProductName' => 'None');
		}
	...
	<body>
		<table >
			...
			<tr><td colspan="2" style="text-align:center;padding-top:10px;"><b>'.$data->Year.' '.$data->Make.' '.$data->Model.'</b></td>  <td class="alignright"><b>Term:</b></td> <td class="alignleft">'; if($NewTerm ==null) {$html.=number_format($data->Term,2); }else { $html.=number_format($NewTerm,2); } $html.='</td> <td class="alignright"><b></b></td> <td class="alignleft"></td></tr>
		</table>';
    ...
    public function load_editProduct()
    {
        ...
        {
            $data = DB::table("ProductPrice")
                         ->where('ProductId', '=', Input::get('ProductId'))
            ...
            else
            {
                ...
                $prices["Ranges"] = array($default, $default, $default);
                $prices["Gap Lease"] = array($default, $default, $default);
                $prices["Gap Balloon"] = array($default, $default, $default);
                $prices["Gap Purchase"] = array($default, $default, $default);
                $prices["SellingPrice"] = array(0, 0, 0);
            }
