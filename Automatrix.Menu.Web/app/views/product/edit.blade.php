...
<div class="row">
	<div class="col-md-9">
		...
		<a class="btn btn-success" id="generalSettings" href="{{URL::action('ProductController@index', array('id' => $dealerId))}}"><i class="fa fa-times"></i> Cancel</a>
	</div>
		...
		<div class="form-group">
			<label for="productNameModified">Product Name</label>
             ...
             @foreach ($products as $productItem)
               <option value="{{{ $productItem->ProductBaseId }}}" @if ( $productItem->ProductBaseId == $productSelected  ) selected="selected" @endif>{{{ $productItem->ProductName }}}</option>
             @endforeach
      </select>
		</div>
		<div class="form-group">
			<label for="displayNameModified">Display Name</label> <input
				...
				class="form-control" value="{{ $product->DisplayName }}" required>
		</div>
		<div class="form-group">
			<label for="ProductDescriptionModified">Product Description</label> <input
				id="ProductDescriptionModified" type="text" class="form-control" value="{{ $product->ProductDescription }}"
				required>
		</div>
		<div class="form-group">
			<label for="bulletsPoints">Bullets Points</label>
			<div class="form-group">
				<input id="bulletPoint1Modified" type="text" class="form-control" @if ( array_key_exists( '0' ,$arrayBullets) ) value="{{ $arrayBullets[0] }}" @endif
					required>
			</div>
			<div class="form-group">
				<input id="bulletPoint2Modified" type="text" class="form-control" @if ( array_key_exists( '1' ,$arrayBullets) ) value="{{ $arrayBullets[1] }}" @endif
					required>
			</div>
			<div class="form-group">
				<input id="bulletPoint3Modified" type="text" class="form-control" @if ( array_key_exists( '2' ,$arrayBullets) ) value="{{ $arrayBullets[2] }}" @endif
					required>
			</div>
			<div class="form-group">
				<input id="bulletPoint4Modified" type="text" class="form-control" @if ( array_key_exists( '3' ,$arrayBullets) ) value="{{ $arrayBullets[3] }}" @endif
					required>
			</div>
			<input id="bulletPoint5Modified" type="text" class="form-control" @if ( array_key_exists( '4' ,$arrayBullets) ) value="{{ $arrayBullets[4] }}" @endif
				required>
		</div>
	</div>
	<div class="col-md-6">
		<ul class="nav nav-tabs" id="optionsCostPrice">
      <li id="manual" @if ($product->UsingWebService == 0) class="active" @endif><a href="#manualtab" data-toggle="tab" id="manuallink">Manual</a></li>
      <li id="webservice" @if ($product->UsingWebService == 1) class="active" @endif><a href="#webservicetab" id="webservicelink" data-toggle="tab">Web Service</a></li>
    </ul>
    <!-- starts tab content -->
    <div class="tab-content" style="padding-top:10px; padding-left:10px;">

      <div @if ($product->UsingWebService == 0) class="tab-pane active" @else class="tab-pane" @endif id="manualtab"><!-- starts manual tab -->
        <div ng-app="app">
        ...
        <fieldset id="range">
          <legend></legend>
          <div class="form-group">
            <label for="useManualPricing"><input type="checkbox" name="useManualPricing" id="useManualPricing" @if ($product->UseRangePricing == true) checked @endif > Range Pricing</label>
          </div>
          <div class="form-group">
            <label for="TypeRange">Type</label>
            <select name="TypeRange" id="TypeRange" class="form-control" style="width:40%;">
              <option value="Gap Purchase" @if ($product->Type == 'Gap Purchase') selected="selected" @endif >Gap Purchase</option>
              <option value="Gap Lease" @if ($product->Type == 'Gap Lease') selected="selected" @endif >Gap Lease</option>
              <option value="Gap Balloon" @if ($product->Type == 'Gap Balloon') selected="selected" @endif >Gap Balloon</option>
            </select>
          </div>
      ...
      <div class="modal fade" id="RangePricingModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" @if ($product->UseRangePricing == 0) hidden @endif >
      ...
      <!-- end modal for manual range pricing -->
        <div @if ($product->UsingWebService == 1) class="tab-pane active" @else class="tab-pane" @endif id="webservicetab">
        <!-- start web service tab -->

          <div id="TypeModifiedProduct" class="form-group">
            <label for="Type">Type</label>
            <span class="pull-right alignText" style="margin-right: 40%;"><input type="checkbox" id="UseTypeModified" name="UseTypeModified" @if ( $product->UseType ) checked @endif > Use option</span>
            <select name="TypeModified" id="TypeModified" class="form-control" style="width:40%" @if ( $product->UseType == 0 ) disabled @endif >
            @if (!empty($types  ))
              @if ($product->ProductBaseId == 4)
                @if (count($types  [0]['type']) >= 2)
                  @foreach ($types  [0]['type'] as $key => $value)
                    <option value="{{ $value['value'] }}" @if ($value['value'] == $product->Type) selected="selected" @endif>{{ $value['text'] }}</option>
                  @endforeach
                @else
         ...
         <div id="TermModifiedProduct" class="form-group">
          ...
          <span class="pull-right alignText" style="margin-right: 40%;"><input type="checkbox" id="UseTermModified" name="UseTermModified" @if ( $product->UseTerm ) checked @endif > Use option</span>
          <select name="TermModified" id="TermModified" class="form-control" style="width:40%" @if ( $product->UseTerm == 0 ) disabled @endif >
          @if (!empty($terms  ))
          @if ($product->ProductBaseId == 9)
            @if (count($terms  [0]['term']) >= 2)
              @foreach ($terms  [0]['term'] as $key => $value)
                <option value="{{ $value['value'] }}" @if ( $value['value'] == $product->Term ) selected="selected" @endif>{{$value['text'] }}</option>
              @endforeach
            @else
        ...
        <div id="DeductibleModifiedProduct" class="form-group">
          ...
          <span class="pull-right alignText" style="margin-right: 40%;"><input type="checkbox" id="UseDeductibleModified" name="UseDeductibleModified" @if ( $product->UseDeductible ) checked @endif > Use option</span>
          <select id="DeductibleModified" name="DeductibleModified" class="form-control"  style="width:40%" @if ($product->UseDeductible == 0) disabled @endif>
          @if (!empty($deductibles))
          @if (count($deductibles[0]['deductible']) > 2)
            @foreach ($deductibles[0]['deductible'] as $key => $value)
              <option value="{{ $value['value'] }}" @if ( $value['value'] == $product->Deductible ) selected="selected" @endif >{{$value['text'] }}</option>
            @endforeach
          @else
        ...
        <div id="VehiclePlanModifiedProduct" class="form-group WSUage2" @if ($productSelected  != 12) hidden @endif>
          ...
          <span class="pull-right alignText" style="margin-right: 40%;"><input type="checkbox" id="UseVehiclePlanModified" name="UseVehiclePlanModified" @if ( $product->UseVehiclePlan ) checked @endif > Use option</span>
          <select id="VehiclePlanModified" name="VehiclePlanModified" class="form-control"  style="width:40%" @if ($product->UseVehiclePlan == 0) disabled @endif >
           <option value="New" @if ($product->VehiclePlan == "New") selected="selected" @endif>New</option>                                
           <option value="PreOwned" @if ($product->VehiclePlan == "PreOwned") selected="selected" @endif>Pre Owned</option>
           <option value="ExtendedEligibilityProgram" @if ($product->VehiclePlan == "ExtendedEligibilityProgram") selected="selected" @endif>Extended Eligibility Program</option>
           <option value="VehiclesWith6monthManufacturersWarranty" @if ($product->VehiclePlan == "VehiclesWith6monthManufacturersWarranty") selected="selected" @endif>Vehicles With 6 month Manufacturers Warranty</option>
           <option value="WrapNew" @if ($product->VehiclePlan == "WrapNew") selected="selected" @endif>Wrap New</option>
           <option value="WrapOemCpo" @if ($product->VehiclePlan == "WrapOemCpo") selected="selected" @endif>Wrap Oem Cpo</option>
           <option value="WrapPreOwned" @if ($product->VehiclePlan == "WrapPreOwned") selected="selected" @endif>Wrap Pre Owned</option>
         </select>
       </div>
       ...
       <div id="MileageModifiedProduct" class="form-group WSUage2"@if ($productSelected  != 2 && $productSelected  != 4 && $productSelected  != 12) hidden @endif>
         <label for="Mileage">Mileage</label>
          <span class="pull-right alignText" style="margin-right: 40%;"><input type="checkbox" id="UseMileageModified" name="UseMileageModified" @if ( $product->UseMileage == 1 ) checked @endif > Use option</span>
          <select id="MileageModified" name="MileageModified" class="form-control"  style="width:40%" @if ( $product->UseMileage == 0 ) disabled @endif >
          @if ( $product->ProductBaseId == 4 )
            <option value="12" @if ($product->Mileage == 12000) selected="selected" @endif >12,000</option>
            <option value="15" @if ($product->Mileage == 15000) selected="selected" @endif >15,000</option>
            <option value="24" @if ($product->Mileage == 24000) selected="selected" @endif >24,000</option>
            <option value="30" @if ($product->Mileage == 30000) selected="selected" @endif >30,000</option>
            <option value="36" @if ($product->Mileage == 36000) selected="selected" @endif>36,000</option>
            <option value="39" @if ($product->Mileage == 39000) selected="selected" @endif>39,000</option>
            <option value="45" @if ($product->Mileage == 45000) selected="selected" @endif>45,000</option>
            <option value="48" @if ($product->Mileage == 48000) selected="selected" @endif>48,000</option>
            <option value="60" @if ($product->Mileage == 60000) selected="selected" @endif>60,000</option>
            <option value="75" @if ($product->Mileage == 75000) selected="selected" @endif>75,000</option>
          @else
            <option value="6" @if ($product->Mileage == 6) selected="selected" @endif >6</option>
            <option value="12" @if ($product->Mileage == 12) selected="selected" @endif >12</option>
            <option value="24" @if ($product->Mileage == 24) selected="selected" @endif >24</option>
            <option value="36" @if ($product->Mileage == 36) selected="selected" @endif >36</option>
            <option value="45" @if ($product->Mileage == 45) selected="selected" @endif>45</option>
            <option value="48" @if ($product->Mileage == 48) selected="selected" @endif>48</option>
            <option value="50" @if ($product->Mileage == 50) selected="selected" @endif>50</option>
            <option value="60" @if ($product->Mileage == 60) selected="selected" @endif>60</option>
            <option value="70" @if ($product->Mileage == 70) selected="selected" @endif>70</option>
            <option value="75" @if ($product->Mileage == 75) selected="selected" @endif>75</option>
            <option value="100" @if ($product->Mileage == 100) selected="selected" @endif>100</option>
            <option value="120" @if ($product->Mileage == 120) selected="selected" @endif>120</option>
            <option value="125" @if ($product->Mileage == 125) selected="selected" @endif>125</option>
            <option value="150" @if ($product->Mileage == 150) selected="selected" @endif>150</option>
          @endif
          </select>
       </div>
       ...
       <div id="TireRotationModifiedProduct" class="form-group WSUage2" @if ($productSelected  != 4) hidden @endif>
         ...
         <span class="pull-right alignText" style="margin-right: 40%;"><input type="checkbox" id="UseTireRotationModified" name="UseTireRotationModified" @if ( $product->UseTireRotation == 1 ) checked @endif > Use option</span>
         <select id="TireRotationModified" name="TireRotationModified" class="form-control"  style="width:40%" @if ( $product->UseTireRotation == 0 ) disabled @endif >
          <option value="5000" @if ( $product->TireRotation == 5000 ) selected="selected" @endif >5000</option>
          <option value="6000" @if ( $product->TireRotation == 6000 ) selected="selected" @endif >6000</option>
          <option value="7500" @if ( $product->TireRotation == 7500 ) selected="selected" @endif >7500</option>
        </select>
      </div>
      ...
      <div id="IntervalModifiedProduct" class="form-group WSUage2" @if ($productSelected  != 4) hidden @endif>
         ...
         <span class="pull-right alignText" style="margin-right: 40%;"><input type="checkbox" id="UseIntervalModified" name="UseIntervalModified" @if ( $product->UseInterval == 1 ) checked @endif > Use option</span>
         <select id="IntervalModified" name="IntervalModified" class="form-control"  style="width:40%" @if ( $product->UseInterval == 0 ) disabled @endif >
          <option value="1" @if ( $product->Interval == 1 ) selected="selected" @endif >1</option>
          <option value="2" @if ( $product->Interval == 2 ) selected="selected" @endif >2</option>
          <option value="3" @if ( $product->Interval == 3 ) selected="selected" @endif >3</option>
          <option value="4" @if ( $product->Interval == 4 ) selected="selected" @endif >4</option>
          <option value="5" @if ( $product->Interval == 5 ) selected="selected" @endif >5</option>
        </select>
      </div>

   </div><!-- Ends tab content -->
   <div class="form-group">
    <label for="useWS"><input type="checkbox" id="useWS" name="useWS" @if ($product->UsingWebService == 1) checked @endif > Use web service to obtain the cost and price</label>
  </div>
  <div id="IsTaxableModifiedProduct" class="form-group WSUage2">
    <label for="IsTaxableModified"><input type="checkbox" name="IsTaxableModified" id="IsTaxableModified" @if ($product->IsTaxable == 1) checked @endif > Is Taxable</label>
  </div>
			<fieldset>
      <legend></legend>
      <div class="form-group">
        ...
        <div id="brochureOptionModified">
          ...
          <input id="BrochureImageReferModified" name="BrochureImageReferModified" type="text" placeholder="File" class="form-control pull-left" @if ($product->BrochureImage) value="{{ $product->BrochureImage }}" @endif style="width:80%">
        </div>
...
$(function() {
      $('#BrochureImageModified').uploadify({
        ...
        'swf'      : currentUrl + '/js/uploadify.swf',
        'uploader' : currentUrl + '/js/uploadify.php',
        ...
        'buttonImage' : currentUrl + '/images/fileicon.png',
        'wmode'     : 'transparent',
        'hideButton': true,
        //'uploadLimit' : 1,
        'auto'     : false,
